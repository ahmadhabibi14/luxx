<script lang="ts">
  ...
  async function handlelogin( e: Event | SubmitEvent ): Promise<void> {
    isLoading = true;
    ...
    const actionURL: string = "http://localhost:1414/api/auth/login";
    try {
  ...
  <div class="basis-[55%] bg-white flex justify-center items-center">
    <section class="flex flex-col h-fit w-[55%]">
      ...
      <div class="mt-5 text-center text-sm">
        <p>Don't have account?  <a href="/login" class="text-sky-600 hover:underline">login</a></p>
      </div>
    </section>
  </div>
</div>
